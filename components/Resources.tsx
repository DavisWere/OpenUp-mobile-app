...
const Resources = () => {
  ...
  const fetchBooks = async () => {
    ...
    try {
      const responses = await Promise.all(
        topics.map(topic =>
          axios.get(`https://www.googleapis.com/books/v1/volumes?q=${topic}&key=${API_KEY}`)
        )
      );
  ...
  const getRandomTopics = (count) => {
    const topics = [
      'depression', 'happiness', 'confidence', 'anxiety', 'mindfulness',
      'stress management', 'self-care', 'positive thinking', 'emotional resilience'
    ];
    const randomTopics = [];
  ...
  return (
    <ScrollView contentContainerStyle={styles.mainContainer}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.itemChoices}>
        ...
        <Text
          style={[styles.itemChoice, filter === 'Recommended' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Recommended')}
        >
          Recommended
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Depression' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Depression')}
        >
          Depression
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Happiness' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Happiness')}
        >
          Happiness
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Confidence' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Confidence')}
        >
          Confidence
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Anxiety' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Anxiety')}
        >
          Anxiety
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Mindfulness' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Mindfulness')}
        >
          Mindfulness
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Stress Management' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Stress Management')}
        >
          Stress Management
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Self-care' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Self-care')}
        >
          Self-care
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Positive Thinking' && styles.selectedFilter]}
          onPress={() => handleFilterChange('Positive Thinking')}
        >
          Positive Thinking
        </Text>
        <Text
          style={[styles.itemChoice, filter === 'Emotional Resilience' && styles.selectedFilter]}
      ...
      {selectedResource === 'Videos' && (
        <View style={styles.videoList}>
          {videos.map(video => (
            ...
            >
              <Image source={{ uri: video.snippet.thumbnails.high.url }} style={styles.bookImage} />
